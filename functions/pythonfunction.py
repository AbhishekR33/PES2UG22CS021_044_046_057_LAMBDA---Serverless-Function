...
def execute_function(function_name, timeout=5):
    ...
    function_path = os.path.abspath(f"{function_name}.py")  # Use the provided file

    if not os.path.exists(function_path):
        raise FileNotFoundError(f"Function file {function_path} does not exist.")
    ...
    try:
        ...
        result = subprocess.run(
            [
                ...
                "-v", f"{function_path}:/app/function.py", image
            ],
